...
for i in range(len(cloud_3d)):
    ...
    if i == 0 or i ==15:
        ...
        y = (-(cloud_3d[i].GetY() - P2Y))/YResolution+IMGY if bLeftOrRight else h-((-(cloud_3d[i].GetY()-P2Y))/YResolution+IMGY) 
        ...
        y = (-(cloud_3d[i].GetY() + OffsetY - P2Y))/YResolution+IMGY if bLeftOrRight else h-((-(cloud_3d[i].GetY()-P2Y))/YResolution+IMGY) 
        vv.append(GvVisionAssembly.sc2Vector(x,y))
        guiCross = GvVisionAssembly.GsScriptGuiCross()
    ...
    if i < nn:
        guiText = GvVisionAssembly.GsScriptGuiText()
        guiText.sScriptGuiStyle = guiStyle
        guiText.strText =str(i)+":  "+str(round(cloud_3d[i].GetX(),3))+"; "+str(round(cloud_3d[i].GetY(),3))+"; "+str(round(cloud_3d[i].GetZ(),3))
        guiText.posX = 550 
        guiText.posY = i*20
        guiText.deg = 0.0
        guiArray.Add(guiText)
